...
function mochilaTrei () {
    console.log('\nAqui esta a lista de pokemon da sua mochila:')
    for (let i = 0; i < MochilaDoTreinador.length; i++) {console.log(`${i+1}. ${MochilaDoTreinador[i].nome}`)
    }
}
...
function removerPok () {   
    ...
    for (let i = 0; i < MochilaDoTreinador.length; i++)     {
        console.log(`${i+1}. ${MochilaDoTreinador[i].nome}`)
    }
    const removerPok = readline.questionInt('Digite o numero conforme o pokemon que quer remover: ')
    if (MochilaDoTreinador.length >=removerPok) {
        const pokRemovido = MochilaDoTreinador[removerPok-1];
        MochilaDoTreinador.splice(removerPok-1,1);
        console.log(`\n${pokRemovido.nome} foi removido da mochila.`)
    }
    
    else {console.log('Nananina NAO - invalido');
    }
